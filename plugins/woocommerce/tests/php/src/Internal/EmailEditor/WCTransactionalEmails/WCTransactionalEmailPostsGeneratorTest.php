...
class WCTransactionalEmailPostsGeneratorTest extends \WC_Unit_Test_Case {
	...
	/**
	 * @testdox Should create an auto-draft post carrying the email type and page template meta.
	 */
	public function test_create_auto_draft_creates_auto_draft_with_identity_meta(): void {
		...
		$post_id = $this->email_generator->create_auto_draft( $email );

		$this->assertGreaterThan( 0, $post_id );
		$this->assertSame( 'auto-draft', get_post_status( $post_id ), 'Lazily created email posts must stay auto-draft until published.' );
		$this->assertSame(
			$email_type,
			(string) get_post_meta( $post_id, WCTransactionalEmailPostsManager::EMAIL_TYPE_META_KEY, true ),
			'_wc_email_type meta must link the auto-draft to its email type.'
		);
		$this->assertSame(
	...
	/**
	 * @testdox Should not write the email_type to post_id option mapping when creating an auto-draft.
	 */
	public function test_create_auto_draft_does_not_write_option_mapping(): void {
		$email_type = 'customer_new_account';
		$email      = $this->template_manager->get_email_by_id( $email_type );
		$this->assertInstanceOf( \WC_Email::class, $email );
		...
		$this->assertEmpty(
			...
			'The option mapping must only be written on publish, not when the auto-draft is created.'
		);
	}

	/**
	 * @testdox Should force auto-draft status even when the content-post-data filter sets publish.
	 */
	public function test_create_auto_draft_forces_auto_draft_status_over_filter(): void {
		add_filter(
	 ...
	 */
	public function test_create_auto_draft_stamps_last_core_render_meta(): void {
