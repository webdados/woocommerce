...
class EmailListingRestController extends RestApiControllerBase {
	 ...
	 *
	 * Bulk post generation used to flush rewrite rules; with lazy creation the
	 * flush happens here, once per WC version, so published email post
	 * permalinks (used by the listing Preview action) keep working.
	 *
	 * @var string
	 */
	 ...
	 *
	 * Returns the post for the given email type, creating an auto-draft with the
	...
	public function recreate_email_post( WP_REST_Request $request ) {
		...
		try {
			$post_id = $this->email_template_generator->create_auto_draft( $email );
		} catch ( \Exception $e ) {
			return new WP_Error(
				'woocommerce_rest_email_post_generation_failed',
	...
	/**
	 * Find an existing unpublished (auto-draft or draft) post for the given email type.
	 *
	 * Draft is included because a core REST autosave can promote an auto-draft
	 * to draft; without matching it here, every Edit click would create a
	 * duplicate scratchpad.
	 *
	...
	private function refresh_scratchpad_content( \WP_Post $scratchpad, \WC_Email $email ): void {
		...
		$updated = wp_update_post(
			array(
				...
				// Keep wp_update_post() from re-validating the existing
				// `_wp_page_template` meta against registered templates: the
				// email template is only registered while the editor package is
				// bootstrapped, and this is a content-only refresh (an empty
				// value skips the template handling, leaving the meta as is).
				'page_template' => '',
			),
			true
		);
		...
		if ( null !== WCEmailTemplateSyncRegistry::get_email_sync_config( (string) $email->id ) ) {
	 ...
	 *
	 * Prefers the sync source hash stamped at creation (content untouched when
	 * it still matches); falls back to creation/modification timestamps for
	 * emails that don't participate in template sync.
	 *
