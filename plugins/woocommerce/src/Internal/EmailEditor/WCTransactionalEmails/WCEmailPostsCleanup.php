...
class WCEmailPostsCleanup {
	 ...
	 *   and the creation/modification timestamps show no edit ever happened.
	 *
	...
	private static function was_never_customized( \WP_Post $post, \WC_Email $email ): bool {
		...
		return $post->post_date_gmt === $post->post_modified_gmt || $post->post_date === $post->post_modified;
	}
	...
	private static function email_type_from_option_name( string $option_name ): string {
		return str_replace(
			array( 'woocommerce_email_templates_', '_post_id' ),
			'',
			$option_name
		);
	}
