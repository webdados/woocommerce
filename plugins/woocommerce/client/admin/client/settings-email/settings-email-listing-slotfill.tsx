...
/**
 * "Edit template" entry point. When no `woo_email` post exists yet (lazy post
 * creation), a post is created on click for the first email type and the
 * editor is opened in template mode; with an existing post the server-built
...
const EditTemplateButton = ( {
	...
	if ( ! emailTemplateId || ! firstEmailTypeId ) {
		return null;
	}

	const handleClick = async () => {
		setIsCreatingPost( true );
		const response = await recreateEmailPostRequest( firstEmailTypeId );
		...
		<Fill>
			...
			>
				...
				<EditTemplateButton
					...
					firstEmailTypeId={ emailTypes[ 0 ]?.id ?? null }
				/>
			</div>
